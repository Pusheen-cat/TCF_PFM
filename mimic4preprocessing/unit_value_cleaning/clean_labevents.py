...
float_to_str_keys = ['51463||BACTERIA||labevents', '51512||URINE MUCOUS||labevents', '50856||ACETAMINOPHEN||labevents',
                     '50999||TRICYCLIC ANTIDEPRESSANT SCREEN||labevents',
                     '51075||BENZODIAZEPINE SCREEN, URINE||labevents',
                     '51085||HCG, URINE, QUALITATIVE||labevents', '50880||BENZODIAZEPINE SCREEN||labevents',
                     '50879||BARBITURATE SCREEN||labevents']  #
for key in float_to_str_keys:
    if key in float_value_dict:
        str_value_dict[key] = float_value_dict.pop(key)

str_to_float_keys = ['50827||VENTILATION RATE||labevents', '50856||ACETAMINOPHEN||labevents']  #
...
'''
여기서 dicts_labevents의 float_value_convert 를 생성. 이를 기반으로 다음 코드 실행
'''
...
#!print(str_convert)
'''
여기서 dicts_labevents의 string_value_convert 를 생성. 이를 기반으로 다음 코드 실행
'''
...
from dicts_labevents import string_value_convert, float_value_convert
...
labevents_str_unit_convert = make_unit_convert_to_main(string_unit_value_dict)
print("간단하게 만들어진 string unit converter: ",labevents_str_unit_convert)
...
for itemid, unit_stats in itemid_unit_trimmed_mean.items():
    ...
    if len(convert_map) > 0:
        labevents_float_unit_convert[itemid] = {
            "convert": convert_map
        }

print('')
print(' === unit 이 두개 이상인 경우 convert할지 제거할지 판단하는 dict 만드는것 도와주는 코드 === ')
print(labevents_float_unit_convert)
print('^ 이걸 사용해서 labevents_float_unit_convert 만듬')

##만들어진 labevents_str_unit_convert 및 labevents_float_unit_convert 불러오기
from dicts_labevents import labevents_float_unit_convert, labevents_str_unit_convert
"""
float_value_dict의 key만 사용해서 unit_value_dict으로부터 float_unit_value_dict 을 생성해. 
그리고 float_unit_value_dict 에서 앞서 만든 labevents_float_unit_convert을 사용해서 서로 다른 unit을 변환할거야 
labevents_float_unit_convert는 '225677||Phosphorous||labevents': {'convert': {'nan': 'mg/dL'}}, 이런식으로 생겼는데 
...
for itemid, unit_dict in string_unit_value_dict.items():
    ...
    convert_rule = labevents_str_unit_convert.get(itemid, {})
...
for itemid, unit_dict in float_unit_value_dict.items():
    ...
    convert_rule = labevents_float_unit_convert.get(itemid, {})
    convert_map = convert_rule.get("convert")
    drop_units = set(convert_rule.get("drop", []))
...
'227442||Potassium (serum)||labevents': {inclusion:[##, ##], decimal:#}
이런 형태의 dict이 될거야
...
'''
from dicts_labevents import float_inclusion_dict
...
print("")
print(f'length "float_unit_value_dict_": {len(float_unit_value_dict_)}')  #229
print(f'length "string_unit_value_dict_": {len(string_unit_value_dict_)}')  #25

'''
convert_key 를 사용하여 같은 의학적 측정값을 의미하지만 itemid가 다른 녀석들을 하나로 합친다.
'''
from dicts_labevents import convert_key_float
...
"""d_labitems.csv를 사용해서 각 feature의 검체 종류를 추가한다. 
Blood인 경우에는 따로 추가하지 않고, 그 외의 경우 추가한다.
 (urine) 이런식으로 덧붙이는 것을 원칙으로 하고, 이미 위치가 적혀있다면 따로 추가하지는 않음"""
path = '/path/to/mimiciv/3.1/hosp/d_labitems.csv'
import pandas as pd
df_labevents = pd.read_csv(path)
print(df_labevents["fluid"].unique())
#['Blood' 'Other Body Fluid' 'Ascites' 'Joint Fluid' 'Pleural' 'Stool' 'Urine' 'Bone Marrow' 'Cerebrospinal Fluid' 'Fluid' 'I' 'Q']
# 1. 전부 소문자로
df_labevents["fluid"] = df_labevents["fluid"].str.lower()
# 2~4. 값 치환
replace_map = {
    "other body fluid": "others",
    "cerebrospinal fluid": "csf",
    "bone marrow": "bm",
}
df_labevents["fluid"] = df_labevents["fluid"].replace(replace_map)
itemid_fluid_dict = (
    df_labevents[["itemid", "fluid"]]
    .dropna(subset=["itemid", "fluid"])
    .set_index("itemid")["fluid"]
    .to_dict()
)

def append_fluid_to_name_if_needed(unit_value_dict, itemid_fluid_dict = itemid_fluid_dict):
    new_dict = {}
    ...
    for key, val in unit_value_dict.items():
        ...
        itemid_str, name = parts[0], parts[1]
        ...
        # itemid가 itemid_fluid_dict에 없으면 skip
        if itemid not in itemid_fluid_dict:
            new_dict[key] = val
            continue

        fluid = itemid_fluid_dict[itemid]

        # fluid가 blood면 skip
        if fluid is None or fluid.lower() == "blood":
            new_dict[key] = val
            continue

        # name 안에 fluid 문자열이 이미 있는지 (대소문자 무시)
        if fluid.lower() in name.lower():
            new_dict[key] = val
            continue

        # name 뒤에 " (fluid)" 추가
        new_name = f"{name} ({fluid})"
        new_key = "||".join([parts[0], new_name] + parts[2:])
        new_dict[new_key] = val

    return new_dict

float_unit_value_dict_ = append_fluid_to_name_if_needed(float_unit_value_dict_)
string_unit_value_dict_ = append_fluid_to_name_if_needed(string_unit_value_dict_)

print(float_unit_value_dict_.keys())
print(f'length "float_unit_value_dict_": {len(float_unit_value_dict_)}')  #229
print(f'length "string_unit_value_dict_": {len(string_unit_value_dict_)}')  #25
...
import pickle
save = False
pkl_path = "./data/unit_value_dicts/labevents_unit_value_dict.pkl"
pkl_path_update = "./data/unit_value_dicts/labevents_unit_value_dict_update.pkl"
if save:
    # 저장/로드 대상 dict
    labevents_unit_value_dict = {
        "float": float_unit_value_dict_,
        "string": string_unit_value_dict_,
    }
    with open(pkl_path, "wb") as f:
        pickle.dump(labevents_unit_value_dict, f)
    print(f"Saved to {pkl_path}")
    raise "Done"
else:
    # load
    with open(pkl_path_update, "rb") as f:
        labevents_unit_value_dict = pickle.load(f)
    float_unit_value_dict_ = labevents_unit_value_dict["float"]
    string_unit_value_dict_ = labevents_unit_value_dict["string"]
