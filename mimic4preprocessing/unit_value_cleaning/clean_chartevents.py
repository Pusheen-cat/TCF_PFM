...
float_to_str_keys = ['229679||Type of Catheter (Impella)||chartevents'] #
for key in float_to_str_keys:
    if key in float_value_dict:
        str_value_dict[key] = float_value_dict.pop(key)
        float_err_value_dict.pop(key,None)

str_to_float_keys = ['229278||Sweep (ECMO)||chartevents']  #
for key in str_to_float_keys:
    if key in str_value_dict:
        float_value_dict[key] = str_value_dict.pop(key)

print(f'str_value_dict개수 {len(str_value_dict)} 과 float_value_dict개수 {len(float_value_dict)}를 더하면 전체 개수 {len(value_dict)}가 됨 ')
...
'''
str_value_dict개수 752 과 float_value_dict개수 379를 더하면 전체 개수 1131가 됨 
Float인데 str값이 잘못 들어가 있는 경우:  0
Float 값의 value들을 갖는 item중에 "nan"값이 들어가 있는 경우의 수: 0

'''

"""  2-1. value가 float 형식의 feature들을 찾고 float_value_convert를 만들자! """
...
print(value_convert)
'''
여기서 dicts_chartevents의 float_value_convert 를 생성. 이를 기반으로 다음 코드 실행
'''


"""  2-2. value가 str 형식의 feature들을 찾고 이들 중 오타 등으로 그룹핑 가능한 애들을 처리 """
str_convert = {}
for _idx, (_key, _value) in enumerate(str_value_dict.items()):
    tmp_drop_list = []
    tmp_convert_dict = {}
    print(_idx+1, _key, _value)
    if 'nan' in _value.keys():
        tmp_drop_list.append('nan')
        del _value['nan']
    if '---' in _value.keys():
        tmp_drop_list.append('---')
        del _value['---']

    if sum([tt for tt in _value.values()]) <5000:
        for idx, tt in enumerate(_value.keys()):
            tmp_drop_list.append(tt)
        print('<5k effective candidate')
        continue

    else:
        for idx, tt in enumerate(_value.keys()):
            tmp_convert_dict[tt] = idx+1
            if len(_value) > 10:
                if _value[tt] < 1000:
                    tmp_convert_dict[tt] = 0
                elif idx+1>=10:
                    tmp_convert_dict[tt] = 0
            else:
                if _value[tt] < 1000:
    ...
    str_convert[_key] = {'drop':tmp_drop_list, 'convert':tmp_convert_dict}

#print(str_convert)
'''
여기서 dicts_chartevents의 string_value_convert 를 생성. 이를 기반으로 다음 코드 실행
'''
...
from dicts_chartevents import string_value_convert, float_value_convert
...
'''
converted float unit_value_dict length:  380
converted string unit_value_dict length:  715
'''
...
chartevents_str_unit_convert = make_unit_convert_to_main(string_unit_value_dict)
print("간단하게 만들어진 string unit converter: ",chartevents_str_unit_convert)
...
for itemid, unit_stats in itemid_unit_trimmed_mean.items():
    ...
    if len(convert_map) > 0:
        chartevents_float_unit_convert[itemid] = {
            "convert": convert_map
        }

print('')
print(' === unit 이 두개 이상인 경우 convert할지 제거할지 판단하는 dict 만드는것 도와주는 코드 === ')
print(chartevents_float_unit_convert)
print('^ 이걸 사용해서 chartevents_float_unit_convert 만듬')


##만들어진 outputevents_str_unit_convert 및 outputevents_float_unit_convert 불러오기
from dicts_chartevents import chartevents_float_unit_convert, chartevents_str_unit_convert
"""
float_value_dict의 key만 사용해서 unit_value_dict으로부터 float_unit_value_dict 을 생성해. 
그리고 float_unit_value_dict 에서 앞서 만든 chartevents_unit_convert을 사용해서 서로 다른 unit을 변환할거야 
chartevents_unit_convert는 '225677||Phosphorous||chartevents': {'convert': {'nan': 'mg/dL'}, 'drop':[]}, 이런식으로 생겼는데 
...
for itemid, unit_dict in string_unit_value_dict.items():
    ...
    convert_rule = chartevents_str_unit_convert.get(itemid, {})
...
for itemid, unit_dict in float_unit_value_dict.items():
    ...
    convert_rule = chartevents_float_unit_convert.get(itemid, {})
    convert_map = convert_rule.get("convert")
    drop_units = set(convert_rule.get("drop", []))
...
'''
from dicts_chartevents import float_inclusion_dict,float_binary_inclusion_dict
...
for itemid, unit_dict in float_unit_value_dict_converted.items():
    ...
    if inclusion_rule is None:
        assert itemid in float_binary_inclusion_dict.keys()
        # inclusion 정의 없으면 그대로 유지
        #float_unit_value_dict_[itemid] = unit_dict
        continue
...
def convert_float_binary_keys(counter: Counter) -> Counter:
    new_counter = Counter()
    for k, v in counter.items():
        if isinstance(k, float) and k in (0.0, 1.0):
            new_counter[int(k)] += v
        else:
            new_counter[k] += v
    return new_counter
binary_unit_value_dict_ = {}
for itemid, unit_dict in float_unit_value_dict_converted.items():
    assert len(unit_dict) == 1
    # inclusion rule 확인
    inclusion_rule = float_binary_inclusion_dict.get(itemid)
    if inclusion_rule is None:
        assert itemid in float_inclusion_dict.keys()
        # inclusion 정의 없으면 그대로 유지
        #binary_unit_value_dict_[itemid] = unit_dict
        continue
    ...
    assert len(filtered_counter) <= 2
    filtered_counter = convert_float_binary_keys(filtered_counter)

    binary_unit_value_dict_[itemid] = {
        unit: filtered_counter
    }
...
print("")
print(f'length "float_unit_value_dict_": {len(float_unit_value_dict_)}') #275
print(f'length "binary_unit_value_dict_": {len(binary_unit_value_dict_)}') #105
print(f'length "string_unit_value_dict_": {len(string_unit_value_dict_)}') #715


'''
convert_key 를 사용하여 같은 의학적 측정값을 의미하지만 itemid가 다른 녀석들을 하나로 합친다.
'''
from dicts_chartevents import convert_key_float
...
for itemid, unit_dict in float_unit_value_dict_.items():
    ...
    if itemid in convert_key_float:
        ...
        existing_unit = next(iter(float_unit_value_dict_[target_key].keys()))
        assert existing_unit == target_unit, (
            f"Unit mismatch for {target_key}: "
            f"{existing_unit} vs {target_unit}"
        )
...
print(f'length "float_unit_value_dict_merged": {len(float_unit_value_dict_merged)}')

float_unit_value_dict_ = float_unit_value_dict_merged

"""여기에 f'{my_itemid['GCS - Total'][0]}||GCS - Total||chartevents' 를 추가한다"""
string_unit_value_dict_[f'{my_itemid['GCS - Total'][0]}||GCS - Total||chartevents'] = {'nan': Counter({0: 390000, 1: 200000, 2: 200000, 3: 200000, 4: 200000, 5: 200000, 6: 200000, 7: 200000, 8: 200000, 9: 200000,})}

"""
CSV 사이 곂치는 feature 처리를 위한 중간 저장 및 불러오기 코드
float_unit_value_dict_ / binary_unit_value_dict_ / string_unit_value_dict_ 저장함
"""
import pickle
save = False
pkl_path = "./data/unit_value_dicts/chartevents_unit_value_dict.pkl"
#### --> Update with interchange_csvs.py #####
pkl_path_update = "./data/unit_value_dicts/chartevents_unit_value_dict_update.pkl"
if save:
    # 저장/로드 대상 dict
    chartevents_unit_value_dict = {
        "float": float_unit_value_dict_,
        "binary": binary_unit_value_dict_,
        "string": string_unit_value_dict_,
    }
    with open(pkl_path, "wb") as f:
        pickle.dump(chartevents_unit_value_dict, f)
    print(f"Saved to {pkl_path}")
    raise "Done"
else:
    # load
    with open(pkl_path_update, "rb") as f:
        chartevents_unit_value_dict = pickle.load(f)
    float_unit_value_dict_ = chartevents_unit_value_dict["float"]
    binary_unit_value_dict_ = chartevents_unit_value_dict["binary"]
    string_unit_value_dict_ = chartevents_unit_value_dict["string"]
...
"""
# chartevent만 특별하게 binary float label이 있음. 이걸 마지막에 str로 옮겨서 str처럼 취급 (float_binary_inclusion_dict)
from binning.make_bin import *
weight_th_list = [10]
exp_list = [0, 0.5, 1, 1.5, 2]
csv_name = 'chartevents'
save_rootpath = "./data/"
...
binary_value_dict_ = remove_unit_dict(binary_unit_value_dict_)
...
result_binary, result_save_binary = create_percentile_bins_str(binary_value_dict_, bin = 10, tag = 'binary')
...
for weight_th in weight_th_list:
    for exp_ in exp_list:
        ...
        result_save = result_string|result_binary|result_float
...
binary_stats, _ = mean_std_feature(binary_value_dict_)
float_stats , _ = mean_std_feature(float_value_dict_)
result_stats = string_stats|binary_stats|float_stats
...
total_dict = float_value_dict_|string_value_dict_|binary_value_dict_
