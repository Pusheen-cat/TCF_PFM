...
print(value_convert)
'''
여기서 dicts_outputevents의 float_value_convert 를 생성. 이를 기반으로 다음 코드 실행
'''
...
print(str_convert)
'''
여기서 dicts_outputevents의 string_value_convert 를 생성. 이를 기반으로 다음 코드 실행
'''
...
from dicts_outputevents import string_value_convert, float_value_convert
...
outputevents_str_unit_convert = make_unit_convert_to_main(string_unit_value_dict)
print("간단하게 만들어진 string unit converter: ",outputevents_str_unit_convert)
...
for itemid, unit_stats in itemid_unit_trimmed_mean.items():
    ...
    if len(convert_map) > 0:
        outputevents_float_unit_convert[itemid] = {
            "convert": convert_map
        }

print('')
print(' === unit 이 두개 이상인 경우 convert할지 제거할지 판단하는 dict 만드는것 도와주는 코드 === ')
print(outputevents_float_unit_convert)
print('^ 이걸 사용해서 outputevents_float_unit_convert 만듬')

##만들어진 outputevents_str_unit_convert 및 outputevents_float_unit_convert 불러오기
from dicts_outputevents import outputevents_float_unit_convert, outputevents_str_unit_convert



"""
float_value_dict의 key만 사용해서 unit_value_dict으로부터 float_unit_value_dict 을 생성해. 
그리고 float_unit_value_dict 에서 앞서 만든 outputevents_float_unit_convert을 사용해서 서로 다른 unit을 변환할거야 
outputevents_float_unit_convert는 '225677||Phosphorous||chartevents': {'convert': {'nan': 'mg/dL'}}, 이런식으로 생겼는데 
...
for itemid, unit_dict in string_unit_value_dict.items():
    ...
    convert_rule = outputevents_str_unit_convert.get(itemid, {})
...
for itemid, unit_dict in float_unit_value_dict.items():
    ...
    convert_rule = outputevents_float_unit_convert.get(itemid, {})
    convert_map = convert_rule.get("convert")
    drop_units = set(convert_rule.get("drop", []))
...
'''
from dicts_outputevents import float_inclusion_dict
...
from dicts_outputevents import convert_key_float
...
import pickle
save = False
pkl_path = "./data/unit_value_dicts/outputevents_unit_value_dict.pkl"
pkl_path_update = "./data/unit_value_dicts/outputevents_unit_value_dict_update.pkl"
if save:
    # 저장/로드 대상 dict
    outputevents_unit_value_dict = {
        "float": float_unit_value_dict_,
        "string": string_unit_value_dict_,
    }
    with open(pkl_path, "wb") as f:
        pickle.dump(outputevents_unit_value_dict, f)
    print(f"Saved to {pkl_path}")
    raise "Done"
else:
    # load
    with open(pkl_path_update, "rb") as f:
        outputevents_unit_value_dict = pickle.load(f)
    float_unit_value_dict_ = outputevents_unit_value_dict["float"]
    string_unit_value_dict_ = outputevents_unit_value_dict["string"]
