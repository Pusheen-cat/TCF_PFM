...
def assemble_episodic_data(stays, diagnoses):
    data = {'Hadm_id': stays.HADM_ID, 'Icustay': stays.STAY_ID, 'Age': stays.AGE, 'Length of Stay': stays.LOS,
    ...
    data = data[['Icustay', 'Ethnicity', 'Gender', 'Age', 'Height', 'Weight', 'Length of Stay', 'Mortality']]
...
def extract_diagnosis_labels(diagnoses):
    ...
    diagnoses['VALUE'] = 1
    labels = diagnoses[['HADM_ID', 'ICD9_CODE', 'VALUE']].drop_duplicates()\
                      .pivot(index='HADM_ID', columns='ICD9_CODE', values='VALUE').fillna(0).astype(int)
    # for l in diagnosis_labels:
    #     if l not in labels:
    #         labels[l] = 0
    # labels = labels[diagnosis_labels]
    # 👉 diagnosis_labels에 맞게 컬럼을 한 번에 맞춤 (없는 건 0으로)
...
def make_phenotype_label_matrix_v2(phenotypes, stays=None):
