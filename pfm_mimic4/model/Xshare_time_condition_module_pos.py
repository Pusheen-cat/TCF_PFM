...
def Xshare_gen_labelset_pos(timestamps, token_types, label, multi_label = 10, multi_label_threshold_sec = 24*3600):
    """
    ...
    mask_nexttoken_type_02 = torch.ones_like(token_types).long()

    if (multi_label == 0) or (multi_label_threshold_sec <1):
        time_next_in_sec[:,:5] = 0
        label[:, :5] = -100
