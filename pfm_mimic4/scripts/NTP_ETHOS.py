#!/usr/bin/env python3
"""
ETHOS baseline launcher (MIMIC-IV).

Next-token prediction with ETHOS time-interval tokens (no RoPE).
Runs pfm_mimic4/main.py under torchrun (DDP), sweeping the learning rate.

Usage:
    python pfm_mimic4/scripts/NTP_ETHOS.py --gpus 0,1,2,3 --num-gpus 4
"""
...
def main():
    ...
    # ------------------------------------------------------------------ #
    # EDIT: pretraining tensors (built by csvs_to_tensor.py) and the
    # directory where checkpoints/results are written.
    DATA_DIR = "/path/to/PFM_pretraining/tensor_saved/"
    CHECKPOINT_DIR = "/path/to/result_pretrained/"
    # ------------------------------------------------------------------ #

    # ETHOS baseline (next-token prediction with ETHOS time-interval tokens).
    base_args = {
        "objective": 'NTP',
        "pe_baseline": 'ETHOS',
        "use_temperature_adj": 0,
        "seq_gen": 'addQ',
        "use_rope": 0,
        "share_tokens": 1,
        "batch_size": 38,
        ...
        # Fixed regularization (paper setting; shared with the proposed method).
        "model_dropout": 0.1,
        "mask_feature_p": 0.1,
        "mask_token_p": 0.1,
    ...
    # Sweep the fine-tuning/pretraining learning rate.
    param_grid = {
