#!/usr/bin/env python3
"""
Cross-dataset (eICU) downstream-task evaluation launcher.

Loads a MIMIC-IV pretrained checkpoint and evaluates it on the eICU downstream
tasks (zero-shot transfer) by re-invoking pfm_mimic4/main.py with
--eval_dataset eICU. Requires the eICU downstream tensors built by
pfm_mimic4/dataset/csvs_to_tensor_dt_eicu.py.
...
def main():
    ...
    # ------------------------------------------------------------------ #
    # EDIT: directory that holds the pretrained-checkpoint folders (the
    # CHECKPOINT_DIR from OURS.py / Xshare_OURS.py).
    EVAL_SAVED_PATH = "/path/to/result_pretrained/"
    # EDIT: eICU downstream tensors built by csvs_to_tensor_dt_eicu.py.
    EICU_DATA_DIR = "/path/to/PFM_downstream/tensor_dt_saved_eicu/"
    # EDIT: the MIMIC-IV pretrained run folder(s) to transfer, relative to
    # EVAL_SAVED_PATH (replace <TIMESTAMP> with your own run's timestamp).
    last_folder = [
    ...
    base_args = {
        ...
        "eval_ft_range": 0,  # 0 = linear probe (frozen backbone)
    }

    # Parameter grid for sweep
    param_grid = {
        "eval_load_pretrained": last_folder,
        "eval_finetune_lr": [5e-4, 1e-4],
        "eval_data_dir": [EICU_DATA_DIR],
        "eval_dataset": ['eICU'],
        "eval_zero_shot": [1],
    }
    start_idx = 0
